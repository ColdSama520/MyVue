...
export default {
      ...
      setup() {
        ...
        const getData = () => {
          ...
          axios.get('http://localhost:9090/Course/CourseCommon', { params : tableData })
              //成功返回
              .then(response => {
                console.log(response);
                if(response.status === 200) {
                  ...
                  ElMessageBox.confirm("确定要归档吗？", "提示", {
                      ...
                      .then(() => {
                          if(response.status === 200) {
                              ...
                              router.push('/classmessage_uncommon')
                          }
                          else{
                              return false;
                          }
        ...
        const handleSearch = () => {
          if(searchTable.course_id === '')
            ...
            axios.get('http://localhost:9090/Course/CourseCommonMessage', {params: searchTable})
                //成功返回
                .then(response => {
                  console.log(response);
                  if (response.status === 200) {
